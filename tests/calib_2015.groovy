...
[ SyntheticProductsWithCapletCalibration.CalibrationMethod.INTEGRATED_EXPECTATION,
  ...
  SyntheticProductsWithCapletCalibration.CalibrationMethod.SWAPTION ].each { calibrationMethod ->
    test("pricing_2015_calib/additive_${calibrationMethod.toString().toLowerCase()}") {
        params = [
                numberOfParams: 15,
    ...
    test("pricing_2015_calib/multiplicative_${calibrationMethod.toString().toLowerCase()}") {
        params = [
                numberOfParams: 15,
                useSeperateCorrelationModels: true,
                numberOfFactors: 8,
    ...
    test("pricing_2015_calib/martingale_${calibrationMethod.toString().toLowerCase()}") {
        params = [
                numberOfParams: 15,
                useSeperateCorrelationModels: true,
                numberOfFactors: 8,
